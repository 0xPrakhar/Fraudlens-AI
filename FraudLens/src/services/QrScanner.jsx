...
import { QrCode, Upload, ArrowRight, Activity, ShieldCheck, XCircle, CheckCircle } from "lucide-react";
    ...
    <div className="max-w-4xl mx-auto space-y-8 text-slate-900 dark:text-slate-100 pb-10">
      ...
      <div className="flex flex-col items-center text-center">
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-3 text-sm max-w-lg">
          Upload a QR code image to inspect its destination before scanning. Prevent malicious redirects and hidden payload execution.
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 flex flex-col items-center shadow-sm">
        <label className="w-full h-64 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-cyan-500 dark:hover:border-cyan-500 transition-all group bg-slate-50 dark:bg-slate-950/50">
          <Upload size={40} className="text-slate-400 mb-4 group-hover:text-cyan-500" />
          <span className="font-bold text-slate-600 dark:text-slate-300">Click to upload QR Code</span>
          <span className="text-xs text-slate-400 mt-1">PNG, JPG or JPEG allowed</span>
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
          <div className={`p-6 rounded-3xl border ${result.status === "MALICIOUS" ? "bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-500/30" : "bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-500/30"}`}>
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-xl font-black">{result.status === "MALICIOUS" ? "Malicious QR Detected" : "Safe QR Verified"}</h2>
                <p className="text-xs font-mono opacity-80 mt-1 break-all">{result.decodedUrl}</p>
              </div>
            </div>
            ...
            <div className="mt-6 flex items-center justify-between bg-black/5 dark:bg-black/20 p-4 rounded-xl">
              ...
              <div className="text-right">
                <p className="text-[10px] uppercase font-bold opacity-60">Risk</p>

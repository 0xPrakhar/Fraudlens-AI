...
import { scanTarget } from "../services/api"; // Aapka api service file path
...
export default function QrScanner() {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      // scanTarget function ka use karke 'qr' service type call karein
      const response = await scanTarget("qr", formData);

      console.log("=== QR SCAN RESPONSE ===", response);
      // Backend response structure: response.data.analysis
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 flex flex-col items-center shadow-sm">
        ...
        {isScanning && (
          <div className="mt-8 flex items-center gap-3 text-cyan-600 dark:text-cyan-400 font-bold">
            <Activity className="animate-spin" /> Analyzing QR Code via AI & Backend...
          </div>
        )}
          ...
          <div className="p-6 rounded-3xl border bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800">
            ...
            <div className="mt-6 space-y-4">
              <div className="bg-black/5 dark:bg-black/40 p-4 rounded-xl">
                <p className="text-xs font-bold text-slate-400 uppercase mb-1">Status / Summary</p>
                <p className="text-sm font-semibold">{result.analysis?.overallStatus} - {result.analysis?.summary}</p>
              </div>
              ...
              <div className="bg-black/5 dark:bg-black/40 p-4 rounded-xl overflow-x-auto">
